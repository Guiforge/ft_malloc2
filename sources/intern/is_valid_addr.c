...
/*   Updated: 2018/09/27 11:47:22 by gpouyat          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_block    *is_valid_addr(void *ptr)
{
    if (!g_malloc.tiny || !g_malloc.medium)
        return (NULL);
    else if (ptr >= (void *)g_malloc.tiny && ptr <= g_malloc.tiny_end && find_block(g_malloc.tiny, get_block(ptr), 0))
        return(g_malloc.tiny);
    else if (ptr >= (void *)g_malloc.medium && ptr <= g_malloc.medium_end && find_block(g_malloc.medium, get_block(ptr), 0))
